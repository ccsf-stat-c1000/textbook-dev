...
The IQ level of students at a particular university has an unknown mean $(\mu)$ and a known standard deviation of $\sigma=15$. A simple random sample of 100 students is found to have the sample mean IQ $\bar{x}=115$. A 95% confidence interval for $\mu$ in this case is:
...
In this case, the 95% confidence interval for $\mu$ is:
...
This explains why with a larger sample size the margin of error (which represents how far apart we believe $\bar{x}$ might be from $\mu$ for a given level of confidence) is smaller.
...
:hint: The margin of error scales as $1/\sqrt{n}$; the sample is 9 times larger.
:feedback-0: Correct! $\sqrt{900}/\sqrt{100} = 3$, so the margin of error shrinks by a factor of 3: from 3 to 1.
:feedback-1: Dividing by 9 would be right if n appeared without the square root; it's $1/\sqrt{n}$.
