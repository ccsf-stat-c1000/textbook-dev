...
4. The Addition Rule for Disjoint Events can be naturally extended to more than two events. In other words, if events A, B, and C are disjoint, then P(A or B or C) = P(A) + P(B) + P(C). Similarly, the Multiplication Rule for Independent Events can be naturally extended to more than two independent events: if events A, B, and C are independent, then P(A and B and C) = P(A) $\times$ P(B) $\times$ P(C). The same is true for 4, 5, ... disjoint/independent events.
