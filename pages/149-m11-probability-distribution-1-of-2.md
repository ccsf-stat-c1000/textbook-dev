...
:::{quiz} In the rescaled shoe-size histogram (vertical axis: probability per half size), how is P(9.5 $\leq X \leq 10.5)$ represented?
...
:feedback-1: With the "probability per half size" scale, the heights alone no longer give probabilities—each bar's area (height $\times 0.5)$ does.
