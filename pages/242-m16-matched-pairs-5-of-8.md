...
The small p-value tells us that there is very little chance of getting data like those observed (or even more extreme) if the null hypothesis were true. More specifically, there is less than a 1% chance (0.009 = 0.9%) of obtaining a test statistic of -2.58 (or lower), assuming that 2 beers have no impact on reaction times.

## Step 4: Conclusion in Context

As usual, we draw our conclusion based on the p-value. If the p-value is small, there is a significant difference between what was observed in the sample and what was claimed in $H_0$, so we reject $H_0$ and conclude that the categorical explanatory variable does affect the quantitative response variable as specified in $H_a$. If the p-value is not small, we do not have enough statistical evidence to reject $H_0$. In particular, if a cutoff probability, $\alpha$ (significance level), is specified, we reject $H_0$ if the p-value is less than $\alpha$; otherwise, we do not reject $H_0$.

In our example, the p-value is 0.009, indicating that the data provide enough evidence to reject $H_0$ and conclude that drinking two beers does slow the reaction times of drivers, and thus that drivers are impaired after drinking two beers.

```{admonition} Comment: Using the Wrong Test
:class: important
...
:feedback-0: Correct! Pairing removes the large driver-to-driver differences in baseline reaction time, so the effect of the beers stands out against much less background noise.
:feedback-1: The data were identical in both analyses—only the method changed.
:feedback-2: The paired test doesn't inflate evidence; it correctly uses the design to eliminate irrelevant variability.
* *The paired analysis removes driver-to-driver variability, making the beer effect easier to detect
* The paired analysis used more data
* The paired test always gives smaller p-values, making it less trustworthy
:::
