...
{term}`Shape <shape>`: Sample means closest to 3,500 will be the most common, with sample means far from 3,500 in either direction progressively less likely. In other words, the shape of the distribution of sample means should bulge in the middle and taper at the ends with a shape that is somewhat normal. This, again, is what we saw when we looked at the sample proportions.
:::

```{admonition} Comment
:class: important

The {term}`distribution` of the values of the sample mean ($\bar{x}$) in repeated {term}`samples <sample>` is called the *sampling distribution of* $\bar{x}$.
```
