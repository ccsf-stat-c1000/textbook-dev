...
- To find the probability of *events of the type "A and B"* (interpreted as both A and B occur), we use the General Multiplication Rule: P(A and B) = P(A) $\times$ P(B | A). In the special case when A and B are independent (the occurrence of one event has no effect on the probability of the other occurring; P(B | A) = P(B)) the Multiplication Rule reduces to: P(A and B) = P(A) $\times$ P(B).
