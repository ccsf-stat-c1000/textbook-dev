...
:::{admonition} Example 2: A Convenience Sample
:class: tip
...
:feedback-0: Correct! Viewers select themselves, and those with the strongest opinions are most likely to respond—a volunteer sample, almost guaranteed to be biased.
:feedback-1: A convenience sample is selected by the researcher's convenience; here the respondents select themselves.
:feedback-2: In a simple random sample every group of individuals has an equal chance of selection—that's not the case here.
* *A volunteer sample—people with strong opinions select themselves, so the results are biased
* A convenience sample—it is representative as long as many people respond
* A simple random sample—texting is random enough
:::

:::{quiz} A pollster wants to know how city residents feel about a new stadium, and surveys every 20th person entering a downtown sports store. Which two problems are most serious here?
:hint: Think about both where the sampling happens and who is on the "list."
:feedback-0: Correct! The sampling frame (sports store customers) doesn't match the population (all residents), and customers of a sports store are likely biased toward supporting a stadium.
:feedback-1: Systematic selection isn't the core problem—the location guarantees an unrepresentative group no matter how the counting is done.
:feedback-2: A bigger sample of the same store's customers would still be biased; sample size doesn't fix a bad sampling frame.
* *The sampling frame doesn't match the population, and store customers likely favor the stadium
* Counting every 20th person is too systematic to be fair
