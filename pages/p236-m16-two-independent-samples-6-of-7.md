...
## Confidence Interval for $\mu_1 - \mu_2$ (Two-Sample t Confidence Interval)
...
In the NHANES weight example, software reports that a 95% confidence interval for $\mu_1 - \mu_2$ (younger minus older males) is (3.1, 6.7) kg.
...
:::{quiz} A different study reports a 95% confidence interval for $\mu_1 - \mu_2$ of (-1.2, 4.8). What does this tell us about the two-sided test of $H_0$: $\mu_1 - \mu_2 = 0$ at the 0.05 level?
:hint: Is 0 inside the interval?
:feedback-0: Correct! Since 0 is inside the interval, it is a plausible value for the difference, so $H_0$ cannot be rejected.
:feedback-1: 0 IS inside (-1.2, 4.8), so we cannot reject $H_0$.
:feedback-2: The interval tells us exactly what the two-sided test at the matching level would conclude.
* *$H_0$ would not be rejected—0 is a plausible value for the difference
* $H_0$ would be rejected—the interval shows a difference exists
