...
:::{admonition} Example 1: SAT-M Scores at Ross College $(\sigma$ Unknown)
:class: tip
...
:::{admonition} Example 2: Concentration of a Chemical in a Drug $(\sigma$ Unknown)
:class: tip
...
*Step 3:* The p-value is the combined area in both tails of the $t(99)$ distribution beyond $\pm 2.5$. Using statistical software, we calculate a p-value of 0.014 with a 95% confidence interval of (244.619, 249.381). For comparison purposes, the output we got when we carried out the z-test for the same problem was a p-value of 0.012 with a 95% confidence interval of (244.648, 249.352).

Note that here the difference between the p-values is quite negligible (0.002). This is not surprising, since the sample size is quite large (n = 100), in which case, as we mentioned, the z-test (in which we treat s as the known $\sigma)$ is a very good approximation to the t-test. Note also how the two 95% confidence intervals are similar (for the same reason).
...
:feedback-0: Correct! The t-test's null distribution is t(n - $1) = t(124)$; the p-value is the two-tailed area beyond $\pm 1.71$.
:feedback-1: $\sigma$ was unknown (the StDev shown is the sample standard deviation), so the t distribution is used.
...
1. In hypothesis testing for the population mean $(\mu)$, we distinguish between two cases: the less common case when the population standard deviation $(\sigma)$ is known, and the more practical case when the population standard deviation is unknown and the sample standard deviation (s) is used instead.

2. In the case when $\sigma$ is known, the test for $\mu$ is called the z-test, and in the case when $\sigma$ is unknown and s is used instead, the test is called the t-test.
...
8. Confidence intervals can be used to carry out the two-sided test ($H_a: \mu \neq \mu_0$), and in cases where $H_0$ is rejected, the confidence interval can give insight into the value of the population mean $(\mu)$.
...
:::{quiz} The test statistic is $t = (9.13 - 12)/(1.11/\sqrt{5}) \approx -5.78$, and the two-sided p-value (under t(4)) is about 0.004. What should Intel conclude?
...
:::{quiz} For the new process, samples of various sizes are collected. In which case can a t-test for $\mu$ safely be used?
...
Consider the two versions of each example $(\sigma$ known vs. $\sigma$ unknown). Write a sentence or two in your own words explaining when the choice between the z-test and t-test makes a real difference, and when it hardly matters.
