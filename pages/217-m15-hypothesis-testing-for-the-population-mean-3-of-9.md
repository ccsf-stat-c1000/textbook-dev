...
The p-value is therefore $2 \times$ P(Z > 2.5). We can either use a table or carry out the test using statistical software. In this case, we get a p-value of 0.012.
:::

## Check Your Understanding: Finding the P-value for a Mean

:::{quiz} A z-test for $\mu$ with alternative $H_a$: $\mu < \mu_0$ produces $z = -1.5$. Which expression gives the p-value?
:hint: A "less than" alternative is left-tailed.
:feedback-0: Correct! For a left-tailed test, the p-value is the area to the left of the observed test statistic: P(Z $\leq -1.5) \approx 0.067$.
:feedback-1: The area to the right of $-1.5 ( \approx 0.933)$ would essentially measure agreement with $H_0$.
...
:hint: Condition on $H_0 (\mu = 250)$ being true, and remember the two-sided alternative.
...
* *If $\mu$ were 250 ppm, the probability of a sample mean as far from 250 as 247 is (in either direction) would be 0.012
* There is a 1.2% chance that the mean concentration is 250 ppm
* If $\mu$ were 250 ppm, the probability of a sample mean of 247 or lower would be 0.012
:::
