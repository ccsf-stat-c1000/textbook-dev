...
This cutoff exists, and because it is so important, it has a special name. It is called the *significance level of the test* and is usually denoted by the Greek letter $\alpha$. The most commonly used significance level is $\alpha = 0.05$ (or 5%). This means that:

- If the p-value < $\alpha$ (usually 0.05), then the data we got are considered to be "rare (or surprising) enough" when $H_0$ is true, and we say that the data provide significant evidence against $H_0$, so we reject $H_0$ and accept $H_a$.
...
- "The results are {term}`statistically significant`" — when the p-value < $\alpha$.
...
*Assessing evidence:* If the proportion of male managers hired is really 0.5 ($H_0$ is true), then the probability that a random selection of three managers will yield three males is $0.5 \times 0.5 \times 0.5 = 0.125$. This is the p-value.
...
:hint: The p-value is a conditional probability: it assumes $H_0$ is true.
...
:feedback-1: The p-value is not the probability that $H_0$ is true—hypotheses are not events with probabilities in this framework.
:feedback-2: The p-value is not the probability that $H_a$ is true.
...
* There is a 0.002 probability that $H_0$ is true
...
:hint: Compare the p-value with $\alpha = 0.05$.
:feedback-0: Correct! Since $0.002 < 0.05$, the data provide significant evidence against $H_0$, so we reject it and conclude that a majority of U.S. adults support gay marriage.
...
:hint: Statistical significance is about the strength of the evidence, judged against $\alpha$.
:feedback-0: Correct! Statistically significant means the p-value fell below the significance level—the data would be quite surprising if $H_0$ were true.
:feedback-1: Statistical significance does not measure the practical size or importance of an effect.
:feedback-2: A significant result provides strong evidence against $H_0$, not certainty.
* *The p-value is below 0.05, so the data provide strong evidence against $H_0$
...
:hint: Condition on $H_0$ being true.
:feedback-0: Correct! If the true average were 12,000 miles, there would be a 0.068 probability of getting sample results like those observed (or more extreme in the direction of $H_a)$.
:feedback-1: The p-value is not the probability that $H_0$ is true.
:feedback-2: This describes $\alpha$, the significance level—not the p-value.
