...
## Check Your Understanding: One-Sided and Two-Sided Alternatives
...
:feedback-0: Correct! The rumors in 2* justify the one-sided $H_a$: $p > 0.157$, while the neutral question in 2** calls for the two-sided $H_a$: $p \neq 0.157$.
...
* *2*: $H_a$: $p > 0.157$; 2**: $H_a$: $p \neq 0.157$
* 2*: $H_a$: $p \neq 0.157$; 2**: $H_a$: $p > 0.157$
* Both use $H_a$: $p > 0.157$
:::
...
## Check Your Understanding: How the Alternative Affects the P-value
...
:feedback-0: Correct! Study II's one-sided p-value is P(Z $\geq 1.9) \approx 0.029$, while study I's two-sided p-value doubles it: $\approx 0.057$.
...
:::{quiz} With $z = 1.9$ and $\alpha = 0.05$, what are the conclusions of the two studies?
:hint: One-sided p-value $\approx 0.029$; two-sided p-value $\approx 0.057$.
:feedback-0: Correct! Study II rejects $H_0 (0.029 < 0.05)$ while study I does not (0.057 > 0.05)—the same data can lead to different conclusions depending on the alternative.
:feedback-1: The two studies reach DIFFERENT conclusions, since only the one-sided p-value falls below 0.05.
:feedback-2: Study I's p-value $( \approx 0.057)$ exceeds 0.05, so it cannot reject $H_0$.
* *Study II rejects $H_0$; study I does not
* Both studies reject $H_0$
* Study I rejects $H_0$; study II does not
:::
