...
Here's how we'll present this material. First, we'll explain what kind of random experiments give rise to a binomial random variable and how the binomial random variable is defined in those types of experiments. We'll then present the probability distribution of the binomial random variable, which will be presented as a formula (which, as you remember, is one of the three ways in which a probability distribution of a discrete random variable can be presented), and explain why the formula makes sense. We'll conclude our discussion by presenting the mean and standard deviation of the binomial random variable.
...
:::{quiz} You flip a coin until you get 3 heads, and X is the number of flips required. Is X binomial?
:hint: Is the number of trials fixed in advance?
:feedback-0: The two outcomes per flip aren't the issue here.
:feedback-1: Correct! The number of trials is not fixed in advance—it depends on when the third head appears—so X is not binomial.
:feedback-2: p is constant at 1/2 for every flip; the problem is the unfixed number of trials.
* No—each flip has more than two outcomes
* *No—the number of trials is not fixed
* No—p changes from flip to flip
:::
