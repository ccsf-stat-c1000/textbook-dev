...
We identified the first example as *theoretical* and the second as *observational*. Let's discuss the distinction. To answer probability questions about a theoretical situation, we only need the principles of probability. However, if we have an observational situation, the only way to answer probability questions is to use the relative frequency we obtain from a random sample.
```
