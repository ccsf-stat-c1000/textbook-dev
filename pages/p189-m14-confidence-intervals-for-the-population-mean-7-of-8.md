...
- Is it always okay to use the confidence interval we developed for $\mu$ when $\sigma$ is known?
...
:feedback-2: D is fine: even with $n = 5$, the population is known to be normal and $\sigma$ is known.
...
As we discussed earlier, when variables have been well-researched in different populations it is reasonable to assume that the population standard deviation $(\sigma)$ is known. However, this is rarely the case. What if $\sigma$ is unknown?
...
for $\mu$ when $\sigma$ is unknown provides a pretty good approximation.
```
