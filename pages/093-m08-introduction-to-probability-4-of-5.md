...
```{figure} images/gen/m08-coin.svg
:alt: The two faces of a coin, heads and tails.
```
...
```{figure} images/gen/m08-dice.svg
...
```{figure} images/gen/m08-spinner.svg
:alt: A spinner in which blue takes up half the circle, and red and yellow each take up a quarter.
```
...
Suppose we had three students—say, two women and one man—and wished to select one of them randomly. To do this you might have each person write his/her name on a (same-sized) piece of paper, then put the three papers in a hat, and select one paper from the hat without looking.
...
:feedback-0: Correct! Each of the three equal sections has probability 1/3.
:feedback-1: 1/2 would be right only if orange took up half the spinner.
:feedback-2: The three probabilities must sum to 1, and each section is the same size.
* *1/3
* 1/2
* 2/3
:::
