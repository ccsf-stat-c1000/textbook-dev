...
If being decided is independent of gender, then 45% of the 60% of the class who are female should have a decided major; in other words, the probability of being female and decided should equal the probability of being female multiplied by the probability of being decided. If the events F and D are independent, we should have P(F and D) = P(F) $\times$ P(D).

In fact, P(F and D) = $27/100 = 0.27$ = P(F) $\times$ P(D) = $0.60 \times 0.45$. This confirms our alternate verification of independence.
:::

In general, another method for checking the independence of events A and B is to compare P(A and B) to P(A) $\times$ P(B). If the two are equal, then A and B are independent; otherwise the two are not independent.
...
:feedback-0: Correct! P(V | X) = 0.6 = P(V); also P(X and V) = 0.2 = P(X) $\times$ P(V) = (1/3)(0.6). All four checks agree: independent.
