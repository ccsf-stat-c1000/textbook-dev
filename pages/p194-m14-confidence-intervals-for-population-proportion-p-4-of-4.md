...
:hint: Check $n\hat{p}$ and $n(1-\hat{p})$.
:feedback-0: Correct! $n\hat{p} = 40(0.075) = 3$, which is far below 10—the normality condition fails, so the usual interval is unreliable.
:feedback-1: $n(1-\hat{p}) = 37$ is fine, but BOTH conditions must hold, and $n\hat{p} = 3$ fails.
* *No—$n\hat{p} = 3$ is less than 10
...
:feedback-1: The conditions hold comfortably: $n\hat{p} = 540$ and $n(1-\hat{p}) = 660$.
