...
- When examining the relationship between two variables, the first step is to classify the two relevant variables according to their role and type, and only then determine the appropriate tools for summarizing the data. (We don't deal with case $Q \to C$ in this course).
